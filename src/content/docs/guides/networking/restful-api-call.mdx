...
int main()
{
    ...
    // Get a product from the REST API
    get_data = http_get(
    ...
    json product = json_from_string(response);

    // Display product information
    write_line("===== Product Information =====");

    write_line(
        "ID          => " +
        std::to_string(
            json_read_number_as_int(product, "id")
        )
    );

    write_line(
        "Title       => " +
        json_read_string(product, "title")
    );

    write_line(
        "Description => " +
        json_read_string(product, "description")
    );

    write_line(
        "Category    => " +
        json_read_string(product, "category")
    );

    write_line(
        "Price       => " +
        std::to_string(
            json_read_number_as_int(product, "price")
        )
    );
