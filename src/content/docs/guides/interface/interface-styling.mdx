...
To use a tinted preset in your own code, simply pass the color as a second argument:

<Tabs syncKey="code-language">
<TabItem label="C++">
```cpp
set_interface_style(SHADED_DARK_STYLE, hsb_color(0.6, 0.4, 1.0));
```
</TabItem>
<TabItem label="C#">
```csharp
SetInterfaceStyle(InterfaceStyle.ShadedDarkStyle, HsbColor(0.6f, 0.4f, 1.0f));
```
</TabItem>
<TabItem label="Python">
```python
set_interface_style(InterfaceStyle.shaded_dark_style, hsb_color(0.6, 0.4, 1.0))
```
</TabItem>
</Tabs>
...
:::tip[Font Availability]
Always ensure the font file exists in your working directory, or use `has_font()` to check before loading to avoid runtime errors.
:::
...
## Part 4: Dynamic Theme Switching

Sometimes you want to let players choose their preferred interface look – dark, light, or even custom colours – **at runtime**. SplashKit makes this easy.

### Basic approach

Just call `set_interface_style()` with the new preset whenever the user triggers a change (e.g., pressing a key or selecting from a dropdown).
...
### Advanced: store a custom theme

If you’ve used the fine‑grained style functions (like `set_interface_color`, `set_interface_font_size`, etc.), you can save and re‑apply those settings:

1. Define a struct (or class) to hold your theme data.
2. When the user picks a theme, apply all settings from that struct.
3. Optionally, use `reset_interface_style()` first to clear any previous customisations.

This pattern is great for settings menus – combine it with `checkbox` and `dropdown` for a polished UX.

:::tip
Check out the [Interface Style Panel](/api/interface/#interface-style-panel) for experimenting with your own theme, then use the same values in your code.
:::
